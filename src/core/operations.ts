...
const put_page: Operation = {
  ...
  handler: async (ctx, p) => {
    ...
    // Skip embedding when the chosen provider is not configured. This avoids
    // retrying a known-missing API key; Ollama counts as configured because it
    // is local and keyless.
    const noEmbed = !isEmbeddingConfigured(ctx.config as GBrainConfig);
